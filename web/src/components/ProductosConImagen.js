        ...
        {productos.map((producto) => (
          <div className="producto-card" key={producto.id_producto}>
            ...
            {cartItems.some(item => item.id === producto.id_producto) ? (
              <button onClick={() => removeFromCart(producto.id_producto)}>Eliminar del carrito</button>
            ) : (
              <button onClick={() => addToCart(producto.id_producto, producto)}>Agregar al carrito</button>
            )}
          </div>
        ))}
      </div>
