...
app.post('/login', async (req, res) => {
  ...
  try {
    ...
    if (rows.length === 1) {
      // Login exitoso, devolver el rol del usuario
      res.json({ success: true, nombre: rows[0].nombre_usuario, rol: rows[0].rol });
    } else {
...
app.post('/reg', async (req, res) => {
  ...
  const { nombre_usuario, correo_electronico, id_ciudad, contrasena, rol } = req.body;
...
app.get('/categorias', async (req, res) => {
  try {
    ...
    const [rows] = await connection.execute('SELECT id_categoria, nombre_categoria FROM categorias_productos');
...
app.get('/departamentos', async (req, res) => {
  try {
    ...
    const [rows] = await connection.execute('SELECT id_departamento, nombre_departamento FROM departamentos');
    ...
    res.status(500).json({ success: false, message: 'Error al obtener la lista de departamentos' });
  }
});
...
app.post('/productos', async (req, res) => {
  ...
  try {
    ...
    await connection.execute(
      ...
      [nombre_producto, descripcion, precio, id_categoria, url_imagen, existencias]
    );
...
app.post('/uploadFiles', async (req,res)=>{
  try {
    if (!req.files) {
      ...
      let fileuri = `uploads/${new Date().getTime()}${avatar.name}`.replace(/[^a-zA-Z0-9.\\\/]+/g, "");
      avatar.mv(fileuri, async err => {
        ...
        res.json({
          base64: `data:${avatar.mimetype};base64, ${fs.readFileSync(fileuri, {encoding: 'base64'})}`,
          url: fileuri.replace(/\\/g, "/")         });
      });
    }
  } catch (err) {
    res.status(500).send({ message: 'Error al procesar archivo, intente de nuevo.' });
  }
});
//--------------------------------
  ...
  try {
    ...
    fs.readdir(uploadsDir, (err, files) => {
      if (err) {
        ...
        res.status(500).json({ success: false, message: 'Error al obtener la lista de imágenes' });
      } else {
        // Crear un array con las rutas completas de las imágenes
        const imagenes = files.map((file) => `http://localhost:5000/uploads/${file}`);
...
app.get('/ciudades/:idDepartamento', async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ success: false, message: 'Error al obtener la lista de ciudades' });
  }
});


const PORT = 5000;
app.listen(PORT, () => console.log(`Servidor Express en ejecución en el puerto ${PORT}`));
